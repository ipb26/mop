...
import { errorAt, formatPath, joinMessages, mapFail, of, orElse, printMessage } from "."
 ...
 */
export const groupMessages = (includeValues = false) => flow(groupErrors(), mapLeft(mapObjIndexed(_ => _.map(_ => printMessage(_, includeValues)))))
/**
 ...
 */
export const combineMessages = (includeValues = false) => flow(groupMessages(includeValues), mapLeft(mapObjIndexed(joinMessages)))
/**
 * Turns the errors into a list of error messages.
 */
export const messageList = (includeValues = false) => flow(combineMessages(includeValues), mapLeft(flow(mapObjIndexed((value, key) => errorAt(key, value)), values)))
/**
 * Turns the errors into one string.
 */
export const singleError = (includeValues = false) => flow(messageList(includeValues), E.mapLeft(l => l.join("\n")))
/**
 ...
 */
export const throwError = () => flow(singleError(true), E.mapLeft(_ => { throw new globalThis.Error(_) }), toUnion)
/**
